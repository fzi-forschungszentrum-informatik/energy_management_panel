...
class TestGetAppNavContentForUser(TestCase):
    ...
    def test_public_page_visable_for_anon_user(self):
        """
        The anon user should have access to the pages for which he got
        explicit permissions.
        """
        app_nav_content = get_app_nav_content_for_user(self.anon_user)
        app_pages = app_nav_content["Demo UI App"]
        self.assertIn(self.expected_page_name_1, app_pages)

    def test_public_page_visable_for_logged_in_user(self):
        """
        Any logged in user should have access to the pages which are available
        for anon users.
        """
